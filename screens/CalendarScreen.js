...
const CalendarScreen = forwardRef((props, ref) => {
  ...
  const fetchDataForSelectedDate = async (date) => {
    try {
      const response = await axios.get(`http://192.168.2.30:4000/journalentry/fetchdateJournalEntries?selectedDate=${date}`);
  ...
  const renderItem = ({ item }) => (
    ...
    >
      <List.Item
        title={item.text}
        left={(props) => (
          <Avatar.Icon {...props} icon={() => <Text style={styles.moodEmoji}>{item.moodEmoji}</Text>} />
