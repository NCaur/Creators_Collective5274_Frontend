...
const SelectMoodScreen = () => {
  ...
  const [markedDates, setMarkedDates] = useState({ [formattedDate]: { selected: true, marked: true } });
  ...
  const handleMoodSelect = async (moodName) => {
    try {
      ...
      if (selectedMood) {
        ...
        const response = await axios.post('http://192.168.2.30:4000/mood/saveMoodEntry', {
          mood: moodName,
        ...
        if (response.data && response.data.message) {
          ...
          navigation.navigate('MakeJournal', {moodEmoji: selectedEmoji, mood: moodName, selectedDate: selectedDate});
        } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.currentTimeContainer}>
        ...
        <Text style={styles.currentTimeText}>{new Date().toLocaleTimeString()}</Text>
      </View>
